...
fn test_save_sample_one() {
    ...
    let my_obj = SampleData {
        ...
        let mut my_service = DynamicVectorManageService:: < SampleData> ::new("sampleData.bin".to_string(), "StringDynamicsampleData.bin".to_string(), 1024).unwrap();
...
fn test_load_sample_one() {
    ...
    let my_service = DynamicVectorManageService:: < SampleData> ::new("sampleData.bin".to_string(), "StringDynamicsampleData.bin".to_string(), 1024).unwrap();
    let i = 1;
...
fn test_load_sample_bulk() {
    ...
    println!("obj at index:{}:\n {:?}", objs.len() -1 ,objs[objs.len() -1]); 
    let duration = start.elapsed(); // 计算时间差
