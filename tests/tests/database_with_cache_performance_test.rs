...
fn test_multi_thread_write_performance() {
    let db = Arc::new(DatabaseWithCache::<
        StaticVectorManageService<TestData>,
        TestData,
    >::new(
...
fn test_random_access_performance() {
    ...
    println!(
        "fill cache with   {} items took: {:?}",
        COUNT, fill_cache_duration
    );
    for _ in 0..TURNS {
    ...
    println!(
        "Random access {} items took: {:?}",
        COUNT,
        random_access_duration - fill_cache_duration
    );
}
